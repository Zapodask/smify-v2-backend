...
export default class PlaylistsController {
    ...
    createPlaylists = async () => {
        try {
            const response = await PlaylistsModel.createPlaylist(this.req.body, this.userId)
    ...
    getPlaylistsMusics = async () => {
        ...
        const response = await PlaylistsModel.getPlaylistsMusics(parseInt(playlist_id))
    ...
    uptadePlaylistStatus = async () => {
        ...
        const response = await PlaylistsModel.updatedPlaylistStatus(playlist_name, id)

        try {
            if (response) {
                this.res.status(200).json('Playlist alterada com sucesso')
            } else {
                this.res.status(401).json({ message: 'Erro ao alterar os dados das playlists' })
            }
        } catch (error) {
            this.res.status(422).json(error)
        }
    ...
    deleteMusicToPlaylist = async () => {
        ...
        const response = await PlaylistsModel.deleteMusicToPlaylist(parseInt(playlist_id as string), parseInt(music_id as string))
